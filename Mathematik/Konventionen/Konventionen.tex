...
\subsection{Ambivalenzen bei Potenzen}

Ist $f$ z.\,B. eine reelle Funktion, dann streitet man sich darüber, was
mit $f^2$ gemeint sein soll, ob das $(f\cdot f)(x):=f(x)f(x)$
ist oder $(f\circ f)(x):=f(f(x))$. Es fällt mir schwer, hier eine
allgemeine Empfehlung auszusprechen, da z.\,B. bei der
Differentialrechnung beide Varianten sinnvoll vorkommen können.
Streng genommen müsste man $f^{\circ 2} := f\circ f$ schreiben,
was man als umständlich empfinden kann. Für mich bietet sich an,
$f^2:=f\circ f$ zu definieren und für $x\mapsto f(x)f(x)$ einfach
$f\! f$ zu schreiben.

\section{Nomenklaturen}
\subsection{Aussagen- und Prädikatenlogik}
Siehe Tabelle \ref{tab:Nomenklatur-pred}. Die booleschen Variablen
sind nullstellige Prädikatvariablen.

\begin{table}[b]\center\small
\begin{tabular}{@{}l|l@{}}
\toprule
\begin{tabular}[t]{@{}ll}
$A,B,C,\ldots$ & boolesche Variablen\\
$\varphi,\psi,\chi,\ldots$ & Formelvariablen\\
$P,Q,R,\ldots$ & Prädikatvariablen\\
$x,y,z,\ldots$ & Individuenvariablen\\
$\Gamma,\Phi,F,M$ & Formelmengen\\
$f,g,h$ & Funktionensymbole\\
$R,S,T$ & Relationensymbole
\end{tabular}
&
\begin{tabular}[t]{ll@{}}
$V$ & Menge der erlaubten booleschen Variablen\\
$\Omega_k$ & Menge erlaubter Operatoren: $\{0,1\}^k\to\{0,1\}$\\
$\Omega$ & $\Omega:=\bigcup_{k=1}^n \Omega_k$\\
$I,J$ & Belegungen, Interpretationen\\
$U$ & Universum, d.\,h. $x,y\in U$\\
$v,w$ & Formelvariablen $v,w\in V$
\end{tabular}\\
\bottomrule
\end{tabular}
\caption{Nomenklatur zur Prädikatenlogik}
\label{tab:Nomenklatur-pred}
\end{table}
