\documentclass[a4paper,10pt,fleqn,twocolumn,twoside,%
openany,dvipdfmx]{scrartcl}
...
\usepackage{geometry}
\geometry{a4paper,left=24mm,right=14mm,top=22mm,bottom=32mm}
...
\newtheoremstyle{defstyle}%
  {}{}{}{}%
  {\normalfont\sffamily\bfseries}{.}{2pt plus 1pt minus 1pt}%
  {}
...
\section*{{\huge Differentialgleichungen}}
...
Für gewöhnlich besitzt eine Differentialgleichung mehr als eine
Lösung. So ist die Funktion $f(x)=c\ee^x$ für jede Konstante $c$ eine
Lösung der Differentialgleichung $f'(x) = f(x)$. Man kann überdies
zeigen, dass jede Lösung von dieser Form sein muss.

Bei einer Problemstellung mit einer eindeutigen Lösung kommt daher
in aller Regel eine Zusatzforderung an die Funktion hinzu, die
unterschiedlicher Art sein kann. Häufig trifft man bei einer
gewöhnlichen Dgl. auf ein
\emph{Anfangswertproblem}, manchmal aber auch auf ein
\emph{Randwertproblem}.

Fordert man zusätzlich, dass $f(x)$ an einer bestimmmten Stelle einen
bestimmten Anfangswert annehmen soll, kurz $f(x_0)=y_0$, bekommt man
für die genannte Gleichung eine eindeutige Lösung, und zwar muss die
Konstante den Wert $c=y_0\ee^{-x_0}$ annehmen.

Bei einer Dgl. höherer Ordnung umfasst das Anfangswertproblem
zusätzlich, welche Werte die Ableitungen der Lösung
an der Stelle annehmen sollen. Beispielsweise ist die Sinusfunktion
$f(x)=\sin(x)$ die eindeutige Lösung des Anfangswertproblems%
\[f''(x) = -f(x),\quad f(0)=0,\quad f'(0)=1.\]
Der Leser möge als kurze Übung beantworten, wie das Anfangswertproblem
für die Kosinusfunktion lautet.
...
\bibitem{Dieudonne} Jean Dieudonné:
