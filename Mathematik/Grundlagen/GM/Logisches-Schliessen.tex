...
Im Fortgang soll $\Gamma\vdash A(x)$ bedeuten, dass die Aussageform
$A(x)$ aus dem Kontext $\Gamma$ ableitbar ist, wobei $x$ beliebig
...
\[\dfrac{\Gamma\vdash A(x)}{\Gamma\vdash\forall x\colon A(x)}?\]
Betrachten wir $x=1\vdash A(x)$. Mit der Regel bekäme man ja
$x=1\vdash\forall x\colon A(x)$. Damit wir nicht durcheinanderkommen,
benennen wir die gebundene Variable $x$ in $y$ um, und erhalten
$x=1\vdash\forall y\colon A(y)$. Wenn die ableitbar ist, dann erst
recht der Fall $x:=1$. Nun braucht man $1=1$ nicht vorauszusetzen.
Somit erhält man $\vdash\forall y\colon A(y)$. Den Quantor beseitigen
wir nun noch mit $y:=x$, und erhalten $\vdash A(x)$. Die Annahme
wurde also aus der Sequenz herausgemogelt. Um dies zu unterbinden,
legen wir fest, dass $x$ keine freie Variable einer der Antezedenzen
sein darf.
