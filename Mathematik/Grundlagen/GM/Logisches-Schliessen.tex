...
Die Behauptung wird in der Form
\[\dfrac{\Gamma\vdash F\Leftrightarrow F'\qquad\Gamma\vdash C(F)}{
\Gamma\vdash C(F')},\quad C(F) := C[X:=F],\quad C(F') := C[X:=F']\]
geschrieben. Wir definieren die Abkürzungen
...
\begin{align*}
\dfrac{\Gamma\vdash A\bicond A'\quad\Gamma\vdash\lnot A}{\Gamma\vdash \lnot A'},\qquad
\dfrac{\Gamma\vdash A\bicond A'\quad\Gamma\vdash B\bicond B'\quad\Gamma\vdash A\circ B}{\Gamma\vdash A'\circ B'},
\end{align*}
wobei $\circ$ jeder der Junktoren $\land,\lor,\cond,\bicond$ sein soll. Es findet sich:
\[
\begin{array}{@{}l@{\qquad}l}
\infer{\Gamma\vdash\lnot A'}{
...
\[
\begin{array}{@{}l@{\qquad}l}
\infer[\infernote{$(x\notin\mathrm{FV}(\Gamma))$}]{\Gamma\vdash\forall x\colon A'}{
  \infer{\Gamma\vdash A'}{
    \infer{\Gamma\vdash A\cond A'}{\Gamma\vdash A\bicond A'}
  & \infer{\Gamma\vdash A}{\Gamma\vdash\forall x\colon A}}}
&
\infer[\infernote{$(x\notin\mathrm{FV}(\Gamma))$}]{\Gamma\vdash\exists x\colon A'}{
  \Gamma\vdash\exists x\colon A
...
gleichwohl auf die herkömmliche zurückführbar. Der Beweis untergliedert
