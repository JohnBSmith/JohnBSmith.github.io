...
\[u\in\{x\in M\mid A(x)\}\defiff u\in M\land A(u).\]
\end{Definition}
...
\[u\in\setsystem U \defiff \exists C\colon u\in C\]
...
definiert gemäß
\[u\in \{x\mid A(x)\} \defiff u\in\setsystem U\land A(u).\]
\end{Definition}
...
dass jede Klasse eine Teilklasse der universelle Klasse sein muss.
...
Verkettung,\index{Komposition}\index{Verkettung}
...
Eine Abbildung heißt bijektiv,\index{bijektiv} wenn sie sowohl injektiv
...
\[(\exists! x\colon A(x)) \iff \exists y\colon \{x\mid A(x)\} = \{y\}.\]
Löst man die Gleichheit darin per Extensionalität auf, findet sich die
kurze Charakterisierung
\[(\exists! x\colon A(x)) \iff \exists y\colon\forall x\colon (A(x)\bicond x=y).\]
...
x\sim y \cond y\sim x, & \text{(Symmetrie)}\\
x\sim y\land y\sim z\cond x\sim z.\quad & \text{(Transitivität)}
\end{array}\]
\end{Definition}
...
Sei $(M,\le)$ halbgeordnet und $A\subseteq M$. Man legt fest
\begin{alignat*}{2}
&\text{$y$ ist ein maximales Element von $A$}\,&&:\bicond\, y\in A\land\lnot\exists x\in A\colon y<x,\\
&\text{$y$ ist ein minimales Element von $A$}\,&&:\bicond\, y\in A\land\lnot\exists x\in A\colon x<y.
\end{alignat*}
\end{Definition}
...
\emph{Hilberts Hotel}\index{Hilberts Hotel} veranschaulicht. Es handelt
...
Die Zimmer in Hilberts Hotel seien mit $1, 2, 3$ usw. nummeriert
