...
System MK, für \emph{Morse"=Kelley}\index{Morse-Kelley-Mengenlehre}.
Gleichwohl bleiben das Axiom der Potenzmenge und das Axiom der Ersetzung
weiterhin verfügbar, um Mengen von echten Klassen abgrenzen zu können.
Wem dieses System zu allgemein ist,  dem steht als strengere Fassung
noch das System NBG, für \emph{Neumann"=Bernays"=Gödel}, zur Verfügung.
Man konnte zeigen, dass jede in NBG beweisbare Aussage, die nur Mengen
...
der Mengen $X,Y$.
\begin{Definition}[Produktmenge]\label{def:Produktmenge}\newlinefirst
...
\begin{Satz}\label{Urbild-unter-Komposition}
...
\end{array}\]
Es stehe $A\tfrac{u}{x}$ als Abkürzung für die Substitution $A[x:=u]$.
Sei also $(n,R)\in\mathcal R$ fest, aber beliebig. Zu zeigen verbleibt
\[R(t_1,\ldots,t_n)\tfrac{u}{x}\cond R(t_1,\ldots,t_n)\tfrac{u}{y},\,
\text{also}\;R(t_1\tfrac{u}{x},\ldots,t_n\tfrac{u}{x})\cond R(t_1\tfrac{u}{y},\ldots,t_n\tfrac{u}{y}).\]
Laut (K2) ist dies gegeben, sobald $t_i\tfrac{u}{x}\equiv t_i\tfrac{u}{y}$
für jedes $i$ gilt. Nun bestätigen wir daher das Schema
\[\forall x,y\in M\colon x\equiv y\cond t\tfrac{u}{x}\equiv t\tfrac{u}{y}\]
via struktureller Induktion über den Termaufbau. Für eine Variable
gilt entweder $v\ne u$, womit sich $v\tfrac{u}{x}\equiv v\tfrac{u}{y}$
zu $v\equiv v$ reduziert, was aus $v=v$ folgt. Oder es gilt $v=u$, womit
sich $v\tfrac{u}{x}\equiv v\tfrac{u}{y}$ zu $x\equiv y$ reduziert, was
...
\[f(t_1,\ldots,t_n)\tfrac{u}{x}\equiv f(t_1,\ldots,t_n)\tfrac{u}{y},\,
\text{also}\;f(t_1\tfrac{u}{x},\ldots,t_n\tfrac{u}{x})\equiv f(t_1\tfrac{u}{y},\ldots,t_n\tfrac{u}{y}),\]
was laut (K1) geht, sobald $t_i\tfrac{u}{x}\equiv t_i\tfrac{u}{y}$
...
\begin{Satz}\label{streng-monoton-und-surjektiv}
...
\begin{Satz}\label{strong-monoton-Aequi}
Seien $(M,\le),(M',\le')$ totalgeordnete Mengen. Ist $f\colon M\to M'$
streng monoton steigend, so gelten die Äquivalenzen
\begin{align*}
...
\end{Satz}
\begin{Beweis}
Laut Satz \ref{streng-monoton-und-surjektiv} ist die Einschränkung
$g\colon M\to f(M)$ mit $g(x):=f(x)$ bijektiv und $g^{-1}$ streng
monoton steigend. Aus $f(x)<f(y)$ folgt somit $x=g^{-1}(f(x))<g^{-1}(f(y))=y$,
also $x<y$. Aus $f(x)\le f(y)$ folgt entsprechend
$x=g^{-1}(f(x))\le g^{-1}(f(y))=y$, also $x\le y$.\,\qedsymbol
\end{Beweis}
...
was aber nichts anderes als das Schema der Induktion\index{Induktion}
ist. Das heißt, wir haben das Schema der Induktion aus der Definition
von $\N$ abgeleitet.
...
\subsection{Wohlfundierter Rekursionssatz}

Auf einer wohlfundierten Menge $(X,R)$ würden wir eine Abbildung
$f\colon X\to Y$ gern rekursiv festlegen dürfen. Man darf sich dabei
die starke Rekursion vorstellen, wenngleich sie hier abstrakter ist. An
der Stelle $x\in X$ ließe sich der Funktionswert $f(x)$ berechnen durch
einen Term, in den nur $x$ und die zuvor bereits bekannten Werte $f(u)$
zu den $u\in X$ mit $R(u,x)$ eingehen.

Wir müssen uns kurz darüber Gedanken machen, wie wir diesen Term formal
ausdrücken. Es muss $f(u)$ in Abhängigkeit von $u$ ausgewählt werden
können. Diese Information enthält aber gerade die Einschränkung von $f$
auf das Anfangsstück $R^{-1}(x)$. Demnach lässt sich die Rekurrenz
ausdrücken als
\[f(x) = \varphi(x,f_{|R^{-1}(x)}),\]
wobei $\varphi\colon X\times\setsystem U\to Y$. Man mag sich fragen,
warum $\varphi$ hier im zweiten Argument auf dem gesamten
Mengenuniversum definiert ist. Dies tun wir schlicht, um den tatsächlichen
Definitionsbereich nicht umständlich notieren zu müssen. Man darf sich
$\varphi$ dabei um irgendeinen Wert fortgesetzt denken, wo sie keine
Menge von Gestalt einer Abbildung auf einem Anfangsstück als Argument
bekommt, was sogar automatisch geschieht, da in der formalen
Mengenlehre alle Operationen auf dem gesamten Klassenuniversum definiert
sind. Im Allgemeinen kommt natürlich nichts Sinnvolles bei heraus, wenn
$g$ im Term $\varphi(x,g)$ eine beliebige Menge ist. Darin besteht aber
kein wirkliches Problem, da die rekursive Berechnung dafür Sorge trägt,
dass $g$ immer eine Abbildung auf dem jeweiligen Anfangsstück sein wird.

\begin{Satz}[Wohlfundierter Rekursionssatz]\newlinefirst
Es sei $(X,R)$ wohlfundiert. Zu festgelegtem
$\varphi\colon X\times\setsystem U\to Y$ von ganz beliebiger Beschaffenheit
besitzt die Rekurrenz
\[\forall x\in X\colon f(x) = \varphi(x,f_{|R^{-1}(x)})\]
immer genau eine Lösung $f\colon X\to Y$.
\end{Satz}

\begin{Beweis}
Zur Eindeutigkeit. Seien $f,g$ zwei Lösungen. Induktion über $x$.
Die Induktionsvoraussetzung ist der äquivalenten Umformung
\begin{align*}
(\forall u\in X\colon R(u,x)\cond f(u)=g(u))
&\iff (\forall u\in R^{-1}(x)\colon f(u)=g(u))\\
&\iff f|_{R^{-1}(x)} = g|_{R^{-1}(x)}.
\end{align*}
zugänglich. Diese Gleichheit ermöglicht nun die Termumformung
\[f(x) = \varphi(x,f_{|R^{-1}(x)}) = \varphi(x,g_{|R^{-1}(x)}) = g(x),\]
die bereits den Abschluss des Induktionsschrittes schafft.

Zur Existenz. Analog wie beim Beweis des dedekindschen Rekursionssatzes
betrachtet man das Mengensystem
\[\begin{array}{@{}r@{}l@{}}
\setsystem A := \{& A\subseteq X\times Y\mid \forall x\in X\colon
\forall g\colon g|_{R^{-1}(x)}\in\Abb(R^{-1}(x),Y)\cond\\
&\quad (\forall u\in R^{-1}(x)\colon (u,g(u))\in A)\cond (x,\varphi(x,g|_{R^{-1}(x)}))\in A\},
\end{array}\]
das aus allen unter dem Rekursionsschritt abgeschlossenen Relationen
besteht. Die Lösung der Rekurrenz erhält man dahingehend als den Schnitt
$f:=\bigcap\setsystem A$. Sie ist die kleinste unter den Relationen,
denn $\setsystem A$ ist nichtleer, weil $X\times Y\in\setsystem A$,
woraufhin man sich auch von $f\in\setsystem A$ überzeugt.
Nun wird bestätigt, dass $f$ der Graph einer Abbildung $X\to Y$
sein muss. Dazu wird
\[\forall x\in X\colon\exists!y\in Y\colon (x,y)\in f\]
vermittels Induktion über $x$ gezeigt. Induktionsvoraussetzung ist
die Existenz genau eines $f(u)$ mit $(u,f(u))\in f$ für jedes $u\in R^{-1}(x)$.
Wegen $f\in\setsystem A$ gilt somit auch $(x,y)\in f$ mit
$y:=\varphi(x,f|_{R^{-1}(x)})$. Angenommen, es gäbe $y'\ne y$ mit $(x,y')\in f$.
Sei $A:=f\setminus\{(x,y')\}$. Es wäre nun $A\in\setsystem A$ im
Widerspruch dazu, dass $f$ die kleinste Menge ist. Für jedes $a\in X$
und $g$ mit $g|_{R^{-1}(X)}\in\Abb(R^{-1}(X),Y)$ gilt nämlich
\[(\forall u\in R^{-1}(a)\colon (u,g(u))\in A)\cond
(a,b)\in A,\quad b := \varphi(a,g|_{R^{-1}(a)}).\]
Dafür muss im Wesentlichen $(a,b)\ne (x,y')$ gezeigt werden.
Angenommen, es gilt $(a,b) = (x,y')$, dann ist $a=x$ und $b=y'$.
Aus der Prämisse, der Allaussage über die $u$, ergibt sich, dass
$f(u)=g(u)$ für jedes $u\in R^{-1}(x)$, und somit
$f|_{R^{-1}(x)}=g|_{R^{-1}(x)}$ per Extensionalität. Dies gelingt
gerade deshalb, weil die Einschränkungen von $f,g$ auf $R^{-1}(x)$ bereits
als Abbildungen gelten, weshalb $(u,g(u))\in f$ auch mit $g(u)=f(u)$
gleichbedeutend ist. Infolge ergibt sich $y=b$, also $y'=y$ im
Widerspruch zur ersten Annahme.

Letztlich verbleibt zu bestätigen, dass $f$ auch die Rekurrenz erfüllt.
Im Zuge der Entfaltung von $f\in\setsystem A$ taucht $(u,f(u))\in f$ bzw.
$f(u)=f(u)$ als Prämisse auf. Da diese aufgrund der Reflexivität der Gleichheit
wahr sein muss, gelangt man zur Konklusion $(x,\varphi(x,f|_{R^{-1}(x)}))\in f$,
also zu
\[\forall x\in X\colon f(x) = \varphi(x,f|_{R^{-1}(x)}).\,\qedsymbol\]
\end{Beweis}

\noindent
Ich habe den Beweis kurz gehalten. Ausführen aller technischen Details
würde ihn recht weitläufig machen.

Es ist anzumerken, dass hiermit bislang lediglich die \emph{lokale}
Variante des Rekursionssatzes bewiesen wurde, wie man so sagt. Dies meint,
dass die Lösung $f\colon X\to Y$ auf einer Menge $X$, also nur
\emph{lokal} definiert ist. Eine \emph{globale} Funktion erhielte man,
wenn für $X$ die Allklasse eingesetzt würde. Dies ist allerdings nicht
möglich, da $f$ im Beweis per se als Element einer Klasse vorkommt, also
keine echte Klasse sein darf. Man kann sich sich nun eine Konstruktion
zur globalen Variante des Satzes überlegen, worauf ich hier aber nicht
näher eingehen will.

Wir schaffen die globale Variante des Satzes stattdessen kurzum herbei,
indem wir die Allkasse als Grothendieck"=Universum modellieren, das
als Menge für $X$ eingesetzt werden darf. Insofern erhalten wir die
globale Variante in gewisser Weise als Spezialfall der lokalen, so
absurd das auch erscheinen mag.

Die rekursive Festlegung globaler Funktionen wird im nächsten Abschnitt
benötigt, um als Definitionsbereich die Ordinalzahlen haben zu können,
die eine echte Klasse bilden. Bspw. ist die kumulative Hierarchie durch
die Ordinalzahlen indiziert. Wir spezialisieren den wohlfundierten
Rekursionssatz dazu zum wohlgeordneten bzw. transfiniten.
...
\subsection{Wohlgeordnete Mengen}

\begin{Axiom}[Auswahlaxiom]\newlinefirst
Zu jedem Mengensystem $X$ mit $\emptyset\notin X$ existiert eine
Auswahlfunktion $f$ mit $\dom(f)=X$, das ist eine Funktion mit
$f(x)\in x$ für jedes $x\in X$.
\end{Axiom}

\noindent
Eine Mengenlehre mit Auswahlaxiom ist keine konstruktive, sondern
notwendig klassisch. Zuerst bestätigt wurde dies in kategorieller Form
von Diaconescu, später dann von Goodman und Myhill für die Mengenlehre.
Der super elegante, sehr kurze Beweis verblüfft zudem darin, nicht mit
unermesslich großen Mengen zu arbeiten. Ich will hier die moderne Fassung
wiedergeben.

\begin{Satz}[Satz von Diaconescu-Goodman-Myhill]\newlinefirst
Das Auswahlaxiom impliziert den Satz vom ausgeschlossenen Dritten.
\end{Satz}
\begin{Beweis}
Es sei $A$ eine beliebige Aussage.\footnote{$A$ darf auch eine Aussageform
sein, wobei allerdings $x,y$ nicht frei in $A$ vorkommen dürfen.}
Wir betrachten die beiden Mengen
\[X := \{x\in\{0,1\}\mid A\lor (x = 0)\},\quad Y := \{y\in\{0,1\}\mid A\lor (y = 1)\},\]
die wegen $0\in X$ bzw. $1\in Y$ beide nichtleer sind. Demnach
existiert laut Auswahlaxiom eine Auswahlfunktion
$f\colon\{X,Y\}\to X\cup Y$ mit $f(X)\in X$ sowie $f(Y)\in Y$.
Wegen $f(X)\in\{0,1\}$ sowie $f(Y)\in\{0,1\}$ treten insgesamt nur
vier Fälle für die Funktionswerte auf. Wir nehmen eine Fallunterscheidung vor.

In den beiden Fällen mit $f(X)=1$ gilt $1\in X$, was sich zu $A\lor (1 = 0)$, also zu $A$ vereinfacht.
Mit $A$ gilt erst recht $A\lor\lnot A$.

In den beiden Fällen mit $f(Y)=0$ gilt $0\in Y$, was sich zu $A\lor (0 = 1)$, also zu $A$ vereinfacht.
Mit $A$ gilt erst recht $A\lor\lnot A$.

Im Fall $f(X)=0, f(Y)=1$ muss $\lnot A$ gelten, denn unter Annahme $A$
wäre $f(X)=f(Y)$, also $0=1$, was absurd ist. Mit $\lnot A$ gilt erst recht $A\lor\lnot A$.

Somit ist $A\lor\lnot A$ in jedem der Fälle gezeigt.\,\qedsymbol
\end{Beweis}

\noindent
Dieser Satz verdeutlicht, dass die Beschränkung auf intuitionistische
Logik keineswegs ausreicht, um eine Theorie der konstruktiven Mathematik
zu schaffen. Die Entwicklung eines diesbezüglichen Axiomensystems erfordert
große Sorgfalt, da sich die Theorie durch Unachtsamkeit allzu leicht wieder
ins Klassische verliert.
