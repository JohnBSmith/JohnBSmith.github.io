...
der Zweiwertigkeit}\index{Prinzip der Zweiwertigkeit} geläufig.
...
\bottomrule
\end{tabular}
&\qquad \begin{tabular}{cc@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}c}
\toprule
...
\begin{table}
\centering
\begin{tabular}{cc@{\qquad}c@{\qquad}c@{\qquad}c}
\toprule
...
\[\models A\land B\Rightarrow B\land A.\]
...
\[(\forall x\in M\colon P(x)) \equiv \bigwedge_{k=1}^n P(x_k),\qquad
(\exists x\in M\colon P(x)) \equiv \bigvee_{k=1}^n P(x_k),\]
...
\strong{Operatorrangfolge und Schreibweisen.}
...
rechte Seite zu berücksichtigen.  Wir legen fest, dass alles hinter
dem Doppelpunkt in den Wirkungsbereich des Quantors fallen soll.
Z.\,B. wird die Formel
\[\forall x\in M\colon P(x)\land A\quad
\text{gelesen als}\quad
\forall x\in M\colon (P(x)\land A).\]
Davon zu unterscheiden ist die Formel
\[(\forall x\in M\colon P(x))\land A.\]
Auch in der Typentheorie treten logische Formeln auf. Dort schreibt
man $\forall x.\,P(x)$ anstelle von $\forall x\colon P(x)$, da
der Doppelpunkt dort bereits für das Urteil $x\colon T$ genutzt wird,
was bedeutet, dass $x$ vom Typ $T$ ist.

Daneben gibt es noch die Schreibweisen $(\forall x)P(x)$ und
$(\exists x)P(x)$, die vor allem innerhalb der Logik genutzt werden.
Man schreibt zur Einsparung von Klammern auch kurz $\forall xPx$ oder
$\forall_x P_x$. Hier haben die Quantoren die höchste Rangfolge, werden
also gelesen wie die Negation, das heißt, $(\forall x)\mathrm{Term}$
wird gelesen wie $\neg\mathrm{Term}$, wobei $\mathrm{Term}$ ein
ungeklammerter Term sein soll.

\begin{Satz}[Distributivgesetze]\mbox{}\\*
Ist $M$ endlich, $A$ eine Aussage und $P(x)$ ein Prädikat auf $M$,
dann gilt
\begin{align*}
A\lor(\forall x\in M\colon P(x)) &\equiv (\forall x\in M\colon A\lor P(x)),\\
A\land(\exists x\in M\colon P(x)) &\equiv (\exists x\in M\colon A\land P(x)).
\end{align*}
\end{Satz}
...
\begin{align*}
\neg(\forall x\in M\colon P(x)) &\equiv (\exists x\in M\colon\neg P(x)),\\
\neg(\exists x\in M\colon P(x)) &\equiv (\forall x\in M\colon\neg P(x)).
\end{align*}
\end{Satz}
...
\begin{align*}
(\forall x\in M\colon P(x)\land Q(x))
& \equiv (\forall x\in M\colon P(x))\land(\forall x\in M\colon Q(x)),\\
(\exists x\in M\colon P(x)\lor Q(x))
& \equiv (\exists x\in M\colon P(x))\lor(\exists x\in M\colon Q(x)).
\end{align*}
\end{Satz}
\strong{Beweis.} Induktionsanfang:
\begin{gather*}
...
\begin{align*}
(\forall x\in M\colon \forall x\in N\colon P(x,y))
& \equiv (\forall x\in N\colon\forall x\in M\colon P(x,y)),\\
(\exists x\in M\colon \exists x\in N\colon P(x,y))
& \equiv (\exists x\in N\colon\exists x\in M\colon P(x,y)).
\end{align*}
\end{Satz}
\strong{Beweis.}
Induktionsanfang:
...
Anstelle von $\forall x\in U\colon P(x)$ schreibt man kürzer
$\forall x\colon P(x)$. Anstelle von $\exists x\in U\colon P(x)$ schreibt
man kürzer $\exists x\colon P(x)$. Das Universum darf unendlich sein,
aber nicht leer, es muss immer mindestens ein Element enthalten.
\begin{Definition}[Allquantor]\mbox{}\\*
Es gilt $\forall x\colon P(x)$ genau dann, wenn $P(x)$ für jedes
beliebige $x$ wahr ist.
\end{Definition}
\begin{Definition}[Existenzquantor]\mbox{}\\*
Es gilt $\exists x\colon P(x)$ genau dann, wenn ein $x$ gefunden
...
\begin{Satz}
Es gilt $A\equiv (\forall x\colon A)$ und $A\equiv(\exists x\colon A)$.
\end{Satz}
\strong{Beweis.} Im Fall $A\equiv 0$ ist auch
$\forall x\colon 0$ falsch, da $0$ für kein $x$ erfüllt ist.
Im Fall $A\equiv 1$ ist auch $\forall x\colon 1$ wahr, da $1$ für jedes
...
denn $(\forall x{\in}\emptyset\colon 0)\equiv 1$.

\begin{Satz}[Verallgemeinerte Distributivgesetze]\mbox{}\\*
Es gilt
\begin{align*}
A\lor (\forall x\colon P(x)) &\equiv (\forall x\colon A\lor P(x)),\\
A\land(\exists x\colon P(x)) &\equiv (\exists x\colon A\land P(x)).
\end{align*}
\end{Satz}
\strong{Beweis.} Im Fall $A\equiv 0$ ergibt sich
\[A\lor(\forall x\colon P(x)) \equiv 0\lor(\forall x\colon P(x))
\equiv (\forall x\colon P(x))\equiv (\forall x\colon 0\lor P(x))
\equiv (\forall x\colon A\lor P(x)).\]
Im Fall $A\equiv 1$ ergibt sich
\[A\lor(\forall x\colon P(x))\equiv 1\lor(\forall x\colon P(x))
\equiv 1 \equiv (\forall x\colon 1)
\equiv (\forall x\colon 1\lor P(x))
\equiv (\forall x\colon A\lor P(x)).\]
Für den Existenzquantor ist die Argumentation analog.\;\qedsymbol
...
\end{align*}
\end{Satz}
\strong{Beweis.} Gilt $\forall x\colon P(x)$, dann ist $P(x)\equiv 1$.
Es ergibt sich
\begin{equation}
\neg(\forall x\colon P(x)) \equiv \neg 1 \equiv 0 \equiv (\exists x\colon 0)
\equiv (\exists x\colon\neg 1)\equiv (\exists x\colon\neg P(x)).
\end{equation}
Gilt $\forall x\colon P(x)$ nicht, dann muss es ein $x$ mit
$\neg P(x)\equiv 1$ geben und es gilt
\begin{equation}
\neg(\forall x\colon P(x)) \equiv \neg 0\equiv 1\equiv (\exists x\colon 1)
\equiv (\exists x\colon\neg P(x)).
\end{equation}
Die Argumentation für den Existenzquantor ist analog.\;\qedsymbol
...
Es gilt
\begin{align*}
(\forall x\colon P(x)\land Q(x))
& \equiv (\forall x\colon P(x))\land (\forall x\colon Q(x)),\\
(\exists x\colon P(x)\lor Q(x))
& \equiv (\exists x\colon P(x))\lor (\exists x\colon Q(x)).
\end{align*}
\end{Satz}
...
$(\forall x\colon P(x))\equiv 1$ und $(\forall x\colon Q(x))\equiv 1$.
Somit gilt
\begin{equation}
(\forall x\colon P(x)\land Q(x)) \equiv 1 \equiv
1\land 1 \equiv (\forall x\colon P(x))\land (\forall x\colon Q(x)).
\end{equation}
...
Dann ist auch $(\forall x\colon P(x))\equiv 0$ oder
$(\forall x\colon Q(x))\equiv 0$. Somit ist
\begin{equation}
(\forall x\colon P(x))\land(\forall x\colon Q(x))\equiv 0.
\end{equation}
...
\equiv \neg(\forall x\colon\neg (P(x)\lor Q(x)))\\
\equiv \neg(\forall x\colon\neg P(x)\land \neg Q(x))
\equiv \neg((\forall x\colon\neg P(x))\land (\forall x\colon\neg Q(x)))\\
\equiv \neg(\forall x\colon\neg P(x))\lor\neg(\forall x\colon\neg Q(x))
\equiv (\exists x\colon P(x))\lor(\exists x\colon Q(x)).\;\qedsymbol
\end{gather}
...
\begin{align*}
(\forall x\in M\colon P(x)) &:\equiv (\forall x\colon x\in M\Rightarrow P(x)),\\
(\exists x\in M\colon P(x)) &:\equiv (\exists x\colon x\in M\land P(x)).
\end{align*}
\end{Definition}
Zuweilen schreibt man auch
\begin{align}
(\forall R(x)\colon P(x)) &:\equiv (\forall x\colon R(x)\Rightarrow P(x)),\\
(\exists R(x)\colon P(x)) &:\equiv (\exists x\colon R(x)\land P(x)),
\end{align}
solange klar bleibt, dass $x$ die gebundene Variable ist.
Z.\,B. $\forall x{<}4\colon P(x)$ und ähnlich.
...
\equiv A\lor(\forall x\colon \neg x\in M\lor P(x))
\equiv (\forall x\colon A\lor\neg x\in M\lor P(x))\\
\equiv (\forall x\colon x\in M\Rightarrow A\lor P(x))
\equiv (\forall x\in M\colon A\lor P(x)).
\end{gather*}
...
\equiv (\exists x\in M\colon A\land P(x)).\;\qedsymbol
\end{gather*}
...
\begin{gather*}
\neg(\forall x\in M\colon P(x))
\equiv \neg(\forall x\colon x\in M\Rightarrow P(x))
\equiv \neg(\forall x\colon \neg x\in M\lor P(x))\\
\equiv (\exists x\colon x\in M\land \neg P(x))
\equiv (\exists x\in M\colon\neg P(x)).
\end{gather*}
Die Argumentation für den Existenzquantor ist analog.\;\qedsymbol
...
ist. Als Formel:%
\[A\ne B \implies (\exists x\in A\colon x\notin B)\lor(\exists x\in B\colon x\notin A).\]
...
\[(\forall x\in A\colon x\in B)\land(\forall x\in B\colon x\in A)\implies A=B.\]
...
&\iff (\forall x\in A\colon x\in B)\land(\forall x\in B\colon x\in A)\\
&\iff (\forall x\colon x\in A\Rightarrow x\in B)\land(\forall x\colon x\in B\Rightarrow x\in A)\\
...
\[\{T(x)\mid P(x)\} := \{y\mid\exists x\colon P(x)\land y=T(x)\}.\]
\end{Definition}
...
dann betrachtet man
\[\{T(x)\mid x\in M\land P(x)\} = \{y\mid\exists x\in M\colon P(x)\land y=T(x)\}.\]
Das heißt, für ein auf $M$ definiertes Prädikat $P$ ergibt sich ein
...
definiert gemäß
\[\bigcup M = \bigcup_{A\in M} A := \{x\mid\exists A\colon A\in M\land x\in A\}.\]
\end{Definition}
...
ist definiert gemäß
\[\bigcap M = \bigcap_{A\in M} A := \{x\mid\forall A\colon A\in M\Rightarrow x\in A\}.\]
\end{Definition}
...
\[\bigcap_{A\in M} A :=
\{x\in\bigcup_{A\in M} A\mid\forall A\colon A\in M\Rightarrow x\in A\}.\]
...
= \{x\mid\exists X\colon \exists i\in I\colon X=A_i\land x\in X\}\\
&= \{x\mid\exists X\colon \exists i\in I\colon x\in A_i\}
= \{x\mid\exists i\in I\colon x\in A_i\}.
\end{align*}
Für $I\ne\emptyset$ definiert man entsprechend
\[\bigcap_{i\in I} A_i := \bigcap A(I) = \{x\mid\exists i\in I\colon x\in A_i\}.\]
...
\end{Definition}
Zu beachten ist, dass hier eine Bildmenge vorliegt, das heißt es gilt
\begin{align*}
A\times B &= \{t\mid\exists a\in A\colon \exists b\in B\colon t=(a,b)\}\\
&= \{t\mid\exists a\colon \exists b\colon a\in A\land b\in B\land t=(a,b)\}.
\end{align*}
...
\begin{gather*}
t\in A\times\emptyset \iff \exists a\colon \exists b\colon a\in A\land b\in\emptyset\land t=(a,b).
\end{gather*}
Nun ist aber $b\in\emptyset$ niemals wahr, da die leere Menge keine
Elemente enthält. Demnach ergibt sich
\begin{gather*}
(\exists a\colon \exists b\colon a\in A\land b\in\emptyset\land t=(a,b))
\iff (\exists a\colon \exists b\colon 0) \iff (\exists a\colon 0)\iff 0.
\end{gather*}
...
\[\forall x\in A\colon\exists y\in B\colon (x,y)\in G.\]
2. Zu jedem $x\in A$ gibt es höchstens ein Bild:
\[\forall (x_1,y_1),(x_2,y_2)\in G\colon x_1=x_2\Rightarrow y_1=y_2.\]
...
\iff (\exists i\in I\colon f(x)\in B_i)\\
&\iff (\exists i\in I\colon x\in f^{-1}[B_i])
...
\[f(x)\in B \defiff \exists y\in B\colon y=f(x).\]
Wir müssen nun zeigen
\[f(x)\in B_1\cap B_2 \iff f(x)\in B_1\land f(x)\in B_2.\]
Expandiert man dies, ist zu zeigen
\[(\exists y\colon y\in B_1\land y\in B_2\land y=f(x))
\iff (\exists y\colon y\in B_1\land y=f(x))
\land (\exists y\colon y\in B_2\land y=f(x)).\]
Die Implikation von links nach rechts ist wahr, denn es gilt ganz allgemein
\[(\exists y\colon P(y)\land Q(y)) \implies
(\exists y\colon P(y))\land (\exists y\colon Q(y)).\]
...
\[(\exists x\colon x\in M\land z
= (g\circ f)(x))\iff (\exists y\colon y\in f[M]\land z=g(y)).\]
Auf der rechten Seite ergibt sich nun
\begin{gather*}
(\exists y\colon y\in f[M]\land z=g(y))
\equiv (\exists y\colon (\exists x\colon x\in M\land y=f(x))\land z=g(y))\\
\equiv (\exists y\colon \exists x\colon x\in M\land y=f(x)\land z=g(y))\\
\equiv (\exists x\colon x\in M\land (\exists y\colon y=f(x)\land z=g(y)))\\
\equiv (\exists x\colon x\in M\land z=g(f(x))).\;\qedsymbol
\end{gather*}
...
\[\forall x_1,x_2\in A\colon f(x_1)=f(x_2)\Rightarrow x_1=x_2\]
bzw.
\[\forall x_1,x_2\in A\colon x_1\ne x_2\Rightarrow f(x_1)\ne f(x_2).\]
\end{Definition}
...
\begin{equation}
\prod_{k=1}^n A_k := \{(a_1,\ldots,a_n)\mid\forall k\colon a_k\in A_k\}.
\end{equation}
...
\forall k\in K\colon f(k)\in A_k\}.
\end{equation}
