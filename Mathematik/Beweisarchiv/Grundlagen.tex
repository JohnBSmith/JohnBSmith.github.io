...
\[\vdash A\lor\neg A.\]
\end{Axiom}
Bemerkung: Zur Schaffung von Klarheit wird ein Beweis mit LEM
markiert, wenn transitive Abhängigkeit zu diesem Axiom bestehen.
Verzichtet keiner der Beweise eines Satzes auf LEM, wird der Satz
ebenfalls mit LEM markiert.
...
= \{y\mid \exists x\colon (x\in M\land y=f(x))\}\]
...
\[f^{-1}(M) := \{x\mid f(x)\in M\} = \{x\mid \exists y\in M\colon y=f(x)\}\]
