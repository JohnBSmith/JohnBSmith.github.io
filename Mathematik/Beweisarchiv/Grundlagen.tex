...
Im Einzelnen gilt
\begin{align*}
\text{(1)}\;\;& A\subseteq A, && \text{(Reflexivität)}\\
\text{(2)}\;\;& A\subseteq B\land B\subseteq A \implies A = B, && \text{(Antisymmetrie)}\\
\text{(3)}\;\;& A\subseteq B\land B\subseteq C \implies A\subseteq C. && \text{(Transitivität)}
\end{align*}
\end{Korollar}
\begin{Beweis}
