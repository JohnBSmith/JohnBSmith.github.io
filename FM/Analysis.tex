...
\begin{Definition}
Das \emdef{Cauchy-Produkt} von zwei Reihen $A_m=\sum_{n=0}^m a_n$
und $B_m=\sum_{n=0}^m b_n$ ist definiert durch
\begin{equation}
(A_m)(B_m):=\sum_{n=0}^m c_n
\;\text{mit}\;
c_n := \sum_{k=0}^n a_k b_{n-k}.
\end{equation}
\end{Definition}
\noindent
Das Cauchy-Produkt von zwei absolut konvergenten Reihen ist
absolut konvergent und es gilt
\begin{equation}\label{eq:cauchy-product-limits}
\hspace{-1em}
\lim_{m\to\infty}((A_m)(B_m))_m
= (\lim_{m\to\infty} A_m)(\lim_{m\to\infty} B_m)
\end{equation}
was als
\begin{equation}
\bigg(\sum_{n=0}^\infty c_n\bigg)
=  \bigg(\sum_{n=0}^\infty a_n\bigg)\bigg(\sum_{n=0}^\infty b_n\bigg).
\end{equation}
notiert wird.
